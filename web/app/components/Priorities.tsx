...
const priorities2026: Priority[] = [
  {
    icon: DollarSign,
    titleKey: 'items2026.capitalization.title',
    descriptionKey: 'items2026.capitalization.description',
    metricKey: 'items2026.capitalization.metric',
  },
  {
    icon: Server,
    titleKey: 'items2026.computing.title',
    descriptionKey: 'items2026.computing.description',
    highlight: true,
  },
  {
    icon: Globe,
    titleKey: 'items2026.global.title',
    descriptionKey: 'items2026.global.description',
    metricKey: 'items2026.global.metric',
  },
];
...
const priorities2030: Priority[] = [
  {
    icon: TrendingUp,
    titleKey: 'items2030.lift.title',
    descriptionKey: 'items2030.lift.description',
    metricKey: 'items2030.lift.metric',
  },
  {
    icon: RocketIcon,
    titleKey: 'items2030.unicorns.title',
    descriptionKey: 'items2030.unicorns.description',
    metricKey: 'items2030.unicorns.metric',
    highlight: true,
  },
  {
    icon: UsersIcon,
    titleKey: 'items2030.decentralization.title',
    descriptionKey: 'items2030.decentralization.description',
  },
  {
    icon: Factory,
    titleKey: 'items2030.scaling.title',
    descriptionKey: 'items2030.scaling.description',
    metricKey: 'items2030.scaling.metric',
  },
];
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col xl:flex-row items-start xl:items-end justify-between gap-6 mb-16">
          ...
          >
            ...
            <h2 className="text-4xl md:text-4xl lg:text-5xl font-bold text-[#151515] font-bebas leading-[0.95]">
              {t('title')}{' '}{t('subtitle')} <span className="text-[#5F891D]">2030</span>
            </h2>
          </motion.div>
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="flex justify-start xl:justify-end w-full"
          >
            <div className="inline-flex flex-col sm:flex-row bg-white/70 backdrop-blur rounded-2xl sm:rounded-full p-1.5 w-full sm:w-auto border border-[#151515]/10 shadow-sm">
              {(['2026', '2030'] as const).map((year) => (
                ...
                >
                  ...
                  <span className="relative z-10">{year === '2026' ? t('year2026') : t('year2030')}</span>
                </button>
              ))}
            </div>
          </motion.div>
        ...
        >
          <h3 className="text-2xl md:text-3xl font-bold text-[#151515]/85 font-bebas">
            {activeTab === '2026' ? t('subtitle2026') : t('subtitle2030')}
          </h3>
        </motion.div>
        ...
        <motion.div
          ...
          className={`grid gap-6 ${
            activeTab === '2026' ? 'md:grid-cols-2 lg:grid-cols-3' : 'md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4'
          }`}
        >
                  ...
                  >
                    ...
                    {priority.metricKey && (
                      <div className={`absolute top-6 right-6 text-right ${priority.highlight ? 'text-[#5F891D]' : 'text-[#5F68A5]'}`}>
                        <span className="text-lg md:text-xl font-bold font-bebas">{t(priority.metricKey)}</span>
                      </div>
                    )}
